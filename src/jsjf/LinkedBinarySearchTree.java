...
public class LinkedBinarySearchTree<T> extends LinkedBinaryTree<T> implements BinarySearchTreeADT<T> {
	 ...
	 * @throws EmptyCollectionException if the tree is empty
	 */
	public T removeMax() throws EmptyCollectionException // extra credit
	{
		T result = null;
	 ...
	 * @throws EmptyCollectionException if the tree is empty
	 */
	public T findMax() throws EmptyCollectionException // extra credit
	{
		T result = null;
	 ...
	 * @throws ElementNotFoundException if the target element is not found
	 */
	public T find(T targetElement) throws ElementNotFoundException // extra credit
	{
	 ...
	 */
	public LinkedBinarySearchTree<T> getLeft() // extra credit
	{
		if (root == null || root.left == null)
			throw new ElementNotFoundException("LinkedBinaryTree");
		else {
			LinkedBinarySearchTree<T> subtree = new LinkedBinarySearchTree<>();
			for(BinaryTreeNode<T> node: listElements(root.left)) {
				subtree.addElement(node.getElement());
			}
	 ...
	 */
	public LinkedBinarySearchTree<T> getRight() { //extra credit
		if (root == null || root.right == null)
			throw new ElementNotFoundException("LinkedBinaryTree");
		else {
			LinkedBinarySearchTree<T> subtree = new LinkedBinarySearchTree<>();
			for(BinaryTreeNode<T> node: listElements(root.right)) {
				subtree.addElement(node.getElement());
			}
	...
	private ArrayUnorderedList<BinaryTreeNode<T>> listElements(BinaryTreeNode<T> node) { //method to be called by  getLeft or getRight to give a list of elements in the subtree so they can be added
		ArrayUnorderedList<BinaryTreeNode<T>> elementList = new ArrayUnorderedList<>();
		
		listElements(node, elementList);
		return elementList;
	}
	private void listElements(BinaryTreeNode<T> node, ArrayUnorderedList<BinaryTreeNode<T>> tempList) { //helper method for listElements based on order methods in LinkedBinaryTree
		
		if (node != null) {
			tempList.addToRear(node);
			listElements(node.getLeft(), tempList);
			listElements(node.getRight(), tempList);
		}
	}
	...
	private BinaryTreeNode<T> findNode(T targetElement, BinaryTreeNode<T> next) {
		...
		if (((Comparable) targetElement).compareTo(next.element) < 0)
